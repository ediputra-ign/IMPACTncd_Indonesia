  ...
  R6::R6Class(
    ...
    public = list(
        ...
        clbr <- fread(
          ...
          colClasses = list(
            numeric = c(
              ...
              "t2dm_incd_clbr_fctr", #edited 20260210
              "chd_ftlt_clbr_fctr",
              "stroke_ftlt_clbr_fctr",
             # "t2dm_ftlt_clbr_fctr", #edited 20260210
          ...
          if (
            dim(clbr[
              chd_incd_clbr_fctr == 1 |
                ...
                t2dm_incd_clbr_fctr == 1 | #edited 20260210
                chd_ftlt_clbr_fctr == 1 |
                stroke_ftlt_clbr_fctr == 1 |
              #  t2dm_ftlt_clbr_fctr == 1 | #edited 20260210
        ...
        for (age_ in age_start:self$design$sim_prm$ageH) {
          ...
          unclbr <- open_dataset(file.path(
            ...
            select(
              ...
              "chd_incd",
              "stroke_incd",
              "t2dm_incd" #edited 20260210
            ) %>%
            collect()
          setDT(unclbr)

          # Aggregate across ses and reg before computing rates
          unclbr <- unclbr[,
            .(
              chd_incd = sum(chd_incd),
              stroke_incd = sum(stroke_incd),
              t2dm_incd = sum(t2dm_incd),
              popsize = sum(popsize)
            ),
          ...
          unclbr[, c("intercept_unclbr", "trend_unclbr") := NULL] #edited 20260210

          # Repeat for t2dm #edited 20260210
          tt <- unclbr[
            t2dm_incd > 0,
            as.list(coef(lm(
              log(t2dm_incd) ~ log(year)
            ))),
            by = sex
          ]
          ...
          benchmark <- read_fst(
            file.path("./inputs/disease_burden", "t2dm_incd.fst"),
            columns = c("age", "sex", "year", "mu"),
            as.data.table = TRUE
          )[age == age_, ]
          benchmark[
          ...
          unclbr[is.na(t2dm_incd_clbr_fctr), t2dm_incd_clbr_fctr := 1]
          ...
          unclbr[, `:=`(
            ...
            # t2dm_prvl_correction = t2dm_incd * (t2dm_incd_clbr_fctr - 1), #edited 20260210
            chd_incd = NULL,
            stroke_incd = NULL,
            t2dm_incd = NULL, #edited 20260210
          ...
          clbr[
            ...
            `:=`(
              ...
              t2dm_incd_clbr_fctr = i.t2dm_incd_clbr_fctr
            )
          ]
          ...
          prvl <- open_dataset(file.path(
            ...
            select(
              ...
              "chd_prvl",
              "stroke_prvl"
              #  "t2dm_prvl" #edited 20260210 #Chirs said it is not needed
            ) %>%
            collect()
          setDT(prvl)
          ...
          prvl <- prvl[,
            .(
              chd_prvl = sum(chd_prvl),
              stroke_prvl = sum(stroke_prvl),
              popsize = sum(popsize)
            ),
            keyby = .(age, sex, year, mc)
          ][,
          ...
          prvl[
            ...
            `:=`(
              ...
              #  t2dm_prvl_correction = i.t2dm_prvl_correction #edited 20260210
            )
          ]
          benchmark <- read_fst(
            #ask Chris: so we don't need to do t2dm_mrtl?
          ...
          if (age_ > age_start) {
            ...
            mrtl <- mrtl[,
              .(
                chd_deaths = sum(chd_deaths),
                stroke_deaths = sum(stroke_deaths),
                nonmodelled_deaths = sum(nonmodelled_deaths),
                popsize = sum(popsize)
              ),
              keyby = .(age, sex, year, mc)
            ][,
      ...
      validate = function() {
        ...
        data_pop <- read_fst(
          "./inputs/pop_projections/combined_population_indonesia_final.fst", #edited 20260210
        ...
        mdd <- mdd[,
          .(nonmodelled_deaths = sum(nonmodelled_deaths),
            ...
            stroke_deaths = sum(stroke_deaths),
            popsize = sum(popsize)),
          keyby = .(mc, scenario, year, agegrp, sex)
        ]
        mdd[, `:=`(
        ...
        mdd[, `:=`(
          ...
          t2dm_incd_rate = t2dm_incd / popsize
        )]
        mdd <- mdd[,
          .(
            ...
            t2dm_incd_rate = quantile(t2dm_incd_rate, p = 0.500),
            t2dm_incd_rate_low = quantile(t2dm_incd_rate, p = 0.025),
            t2dm_incd_rate_upp = quantile(t2dm_incd_rate, p = 0.975),
            type = "modelled"
          ),
          keyby = .(year, agegrp, sex)
        ]
        ...
        absorb_dt(obs, tt)
        ttt <- read_fst(
          paste0("./inputs/disease_burden/", "t2dm_incd.fst"),
          columns = c("age", "year", "sex", "mu", "mu_lower", "mu_upper"),
          as.data.table = TRUE
        )
        setnames(
          ttt,
          c("mu", "mu_lower", "mu_upper"),
          c("t2dm_incd_rate", "t2dm_incd_rate_low", "t2dm_incd_rate_upp")
        )
        absorb_dt(obs, ttt)
        absorb_dt(obs, data_pop)
        to_agegrp(obs, 5, 99)
        ...
        #edited 20260210 #added t2dm
        p <- ggplot() +
          geom_line(
            data = dt[sex == "men"],
            aes(x = year, y = t2dm_incd_rate, color = type)
          ) +
          geom_line(
            data = dt[sex == "men"],
            aes(x = year, y = t2dm_incd_rate_low, color = type),
            linetype = "dashed"
          ) +
          geom_line(
            data = dt[sex == "men"],
            aes(x = year, y = t2dm_incd_rate_upp, color = type),
            linetype = "dashed"
          ) +
          facet_wrap(. ~ factor(agegrp), scales = "free") +
          theme_bw() +
          theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
          ggtitle("T2DM incd rate", "Men")
        ggsave(
          file.path(
            self$design$sim_prm$output_dir,
            "plots",
            "t2dm_as_men_incd.jpg"
          ),
          p,
          height = HEIGHT,
          width = WIDTH
        ...
        p <- ggplot() +
          ...
          theme_bw() +
          theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
          ggtitle("T2DM incd rate", "Women")
        ggsave(
          file.path(
            self$design$sim_prm$output_dir,
            "plots",
            "t2dm_as_women_incd.jpg"
          ),
          p,
          height = HEIGHT,
          width = WIDTH
        ...
        #edited 20260210 #added for t2dm
        p <- ggplot() +
          geom_line(
            data = dt,
            aes(x = year, y = t2dm_incd_rate, color = type)
        ...
        obs <- read_fst(
          ...
          columns = c(
            ...
            "mu_lower",
            "mu_upper"
           # "prvl_mltp" #edited 20260211 prvl_mltp is not included
          ),
          as.data.table = TRUE
        )
        setnames(
        ...
        obs[, `:=`(
          chd_prvl_rate = chd_prvl_rate, #* prvl_mltp, #edited 20260211 prvl_mltp is not included
        ...
        tt[, `:=`(
          stroke_prvl_rate = stroke_prvl_rate, #* prvl_mltp, #edited 20260211 prvl_mltp is not included
          stroke_prvl_rate_low = stroke_prvl_rate_low, #* prvl_mltp,
          stroke_prvl_rate_upp = stroke_prvl_rate_upp #* prvl_mltp,
          #prvl_mltp = NULL
        )]

        #edited 20260210 #Chris said td2m_prvl is not needed
        absorb_dt(obs, tt)
        # ttt <- read_fst(
        #   paste0("./inputs/disease_burden/", "t2dm_prvl.fst"),
        #   columns = c(
        #     "age",
        #     "year",
        #     "sex",
        ...
        #   tt,
        #   c("mu", "mu_lower", "mu_upper"),
        #   c("t2dm_prvl_rate", "t2dm_prvl_rate_low", "t2dm_prvl_rate_upp")
        # )
        # ttt[, `:=`(
        #   t2dm_prvl_rate = t2dm_prvl_rate * prvl_mltp, #edited 20260210 here we still need prvl_mltp
        #   t2dm_prvl_rate_low = t2dm_prvl_rate_low * prvl_mltp,
        #   t2dm_prvl_rate_upp = t2dm_prvl_rate_upp * prvl_mltp,
        #   prvl_mltp = NULL
        # )]
        # absorb_dt(obs, ttt)
        absorb_dt(obs, data_pop)
        ...
        #   geom_line(
        #     data = dt[sex == "men"],
        #     aes(x = year, y = t2dm_prvl_rate, color = type)
        #   ) +
        #   geom_line(
        #     data = dt[sex == "men"],
        #     aes(x = year, y = t2dm_prvl_rate_low, color = type),
        #     linetype = "dashed"
        #   ) +
        #   geom_line(
        #     data = dt[sex == "men"],
        #     aes(x = year, y = t2dm_prvl_rate_upp, color = type),
        #     linetype = "dashed"
        #   ) +
        #   facet_wrap(. ~ factor(agegrp), scales = "free") +
        #   theme_bw() +
        #   theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
        #   ggtitle("T2DM prvl rate", "Men")
        # ggsave(
        #   file.path(
        #     self$design$sim_prm$output_dir,
        #     "plots",
        #     "t2dm_as_men_prvl.jpg"
        #   ),
        #   p,
        #   height = HEIGHT,
        #   width = WIDTH
        ...
        #   theme_bw() +
        #   theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
        #   ggtitle("T2DM prvl rate", "Women")
        # ggsave(
        #   file.path(
        #     self$design$sim_prm$output_dir,
        #     "plots",
        #     "t2dm_as_women_prvl.jpg"
        #   ),
        #   p,
        #   height = HEIGHT,
        #   width = WIDTH
    ...
    private = list(
      ...
      export_xps = function(sp, scenario_nam) {
        ...
        if ("Smoking_curr_xps" %in% names(sp$pop)) {
          sp$pop[,
        ...
        if ("PA_days_curr_xps" %in% names(sp$pop)) {
          sp$pop[,
      ...
      if ("SBP_curr_xps" %in% names(sp$pop)) { # TODO Set to FALSE when JPN21 project is over
        ...
        # Tidy up
        tidy_cols <- intersect(
          c("agegrp10", "smok_never_curr_xps", "smok_active_curr_xps",
            "smok_active_lagged", "pa567_curr_xps"),
          names(sp$pop)
        )
        if (length(tidy_cols) > 0) sp$pop[, (tidy_cols) := NULL]
        if ("smok_cess_rate_curr_xps" %in% names(sp$pop)) {
      ...
      calc_QALYs = function(
        ...
        available_cols <- dbListFields(duckdb_con, input_table_name)

        # Disease utility decrements: significant terms
        sig_terms <- list(
          chd_prvl    = c(eq5d5l = -0.073, hui3 = -0.081),
          stroke_prvl = c(eq5d5l = -0.265, hui3 = -0.293),
          t2dm_prvl   = c(eq5d5l = -0.046, hui3 = -0.055)
        )
        # Disease utility decrements: non-significant terms
        nonsig_terms <- list(
          htn_prvl     = c(eq5d5l = -0.005, hui3 = -0.006),
          obesity_prvl = c(eq5d5l = -0.034, hui3 =  0.019)
        )

        # Build disease SQL fragments, only for columns that exist
        make_dis_sql <- function(terms, scale) {
          paste0(vapply(names(terms), function(col) {
            if (col %in% available_cols) {
              sprintf(" + CASE WHEN %s = 0 THEN 0.0 ELSE %s END", col, terms[[col]][[scale]])
            } else ""
          }, character(1)), collapse = "")
        }

        # Original age/sex-adjusted Japanese population-norms EQ5D5L formula,
        # kept for reference/rollback - superseded by the flat Indonesian
        # population-average formula below.
        # eq5d5l_expr <- paste0("
        #   0.989
        #   + CASE agegrp
        ...
        #   make_dis_sql(sig_terms, "eq5d5l"),
        #   if (!include_non_significant) make_dis_sql(nonsig_terms, "eq5d5l") else ""
        # )

        # EQ-5D score for the Indonesian population: flat population-average
        # baseline (not age/sex-adjusted) plus disease and BMI-band disutilities.
        #Updated on 20260812
        eq5d5l_expr <- paste0(
          "0.91",
          if ("chd_prvl" %in% available_cols)
            " + CASE WHEN chd_prvl = 0 THEN 0.0 ELSE -0.09 END" else "",
          if ("stroke_prvl" %in% available_cols)
            " + CASE WHEN stroke_prvl = 0 THEN 0.0 ELSE -0.25 END" else "",
          if ("t2dm_prvl" %in% available_cols)
            " + CASE WHEN t2dm_prvl = 0 THEN 0.0 ELSE -0.05 END" else "",
          if ("BMI" %in% available_cols)
      ...
      calc_costs = function(
        ...
        # --- Compute extra strata columns to pass through cost SQL ---
        extra_strata_cols <- setdiff(
          self$design$sim_prm$strata_for_output,
          c("scenario", "year", "agegrp", "sex")
        )
        extra_strata_sql <- if (length(extra_strata_cols) > 0) {
          paste0(", ", paste(extra_strata_cols, collapse = ", "))
        } else ""
        extra_strata_m_sql <- if (length(extra_strata_cols) > 0) {
          paste0(", ", paste0("m.", extra_strata_cols, collapse = ", "))
        } else ""

        # --- Inflation Factors ---
        # Only used by the commented-out Steps 1-3 below. #Updated on 20260812
        # prod_informal_inflation_factor <- 1.025
        # direct_costs_inflation_factor <- 99.6 / 99.7

        # Steps 1-3 below (productivity, informal care, and age/sex-weighted
        # direct cost calculation for chd/stroke) are commented out, not
        # deleted: none of the tables they rely on (employee counts, informal
        # care hours, national cost totals used to allocate per-person costs)
        # have a documented Indonesia-specific source - no `source:`/citation
        # anywhere, unlike e.g. the RR .csvy files. Until real Indonesian data
        # is available, cost = the flat per-case direct cost only (see
        # Step 4's basic_costs CTE), for chd/stroke and t2dm alike.
        ...
        #   # list("t2dm_prvl_2016_agg_view", "t2dm_dgns > 0", 2016), #edited 20260210
        #   # list("t2dm_prvl_2019_agg_view", "t2dm_dgns > 0", 2019), #edited 20260210
...
#           "inputs/pop_estimates/observed_population_indonesia_final.fst",
#           columns = c("year", "age", "sex", "pops"),
#           as.data.table = TRUE
#         )[year == 2016L]
#         obs_pop_2016 <- obs_pop_2016[, .(pops = sum(pops)), keyby = .(year, age, sex)]
#         
#         # Process CHD mortality efficiently
#         chd_ftlt_2016 <- read_fst( #ask Chris: no need for t2dm?
...
#         "
# 
#         # Create all cost parameter views efficiently #Ask Chris about t2dm
#         cost_configs <- list(
#           list(
...
#         "
# 
#         private$execute_sql( #ask Chris: about t2dm?
#           duckdb_con,
#           sprintf(
#             direct_cost_sql,
...
#             SELECT mc, scenario, year, agegrp, sex%s, chd_dgns, all_cause_mrtl, stroke_dgns, t2dm_dgns, wt, wt_esp
#             FROM %s
...
#             basic_costs AS (
#               SELECT
#                 m.mc, m.scenario, m.year, m.agegrp, m.sex%s,
#                 m.wt, m.wt_esp,
# 
...
# 
#                 -- Flat per-case direct costs (USD), separate from the
#                 -- age/sex-weighted direct cost calculation above
#                 CASE WHEN m.t2dm_dgns > 0 THEN 579 ELSE 0 END AS t2dm_direct_cost,
#                 CASE WHEN m.chd_dgns > 0 THEN 7338 ELSE 0 END AS chd_direct_cost,
#                 CASE WHEN m.stroke_dgns > 0 THEN 1685 ELSE 0 END AS stroke_direct_cost
# 
...
#             )
#             SELECT
#               mc, scenario, year, agegrp, sex%s, wt, wt_esp,
# 
...
#               stroke_direct_costs,
# 
#               -- Flat per-case direct costs (USD)
#               t2dm_direct_cost,
#               chd_direct_cost,
#               stroke_direct_cost,
# 
...
# 
#               -- Aggregated direct costs (flat per-case direct costs replace
#               -- the age/sex-weighted direct cost calculation here)
#               (chd_direct_cost + stroke_direct_cost) AS cvd_direct_costs,
# 
...
# 
#               -- Total costs (indirect + direct; direct = flat per-case cost)
#               (chd_prvl_prdv_costs + chd_mrtl_prdv_costs + chd_informal_costs + chd_direct_cost) AS chd_total_costs,
#               (stroke_prvl_prdv_costs + stroke_mrtl_prdv_costs + stroke_informal_costs + stroke_direct_cost) AS stroke_total_costs,
#               (chd_prvl_prdv_costs + chd_mrtl_prdv_costs + chd_informal_costs + chd_direct_cost + stroke_prvl_prdv_costs + stroke_mrtl_prdv_costs + stroke_informal_costs + stroke_direct_cost) AS cvd_total_costs
# 
#             FROM basic_costs;
#         ",
...
#         )
        ...
        # Active version: keeps the ORIGINAL formula shape (total cost =
        # productivity + informal + direct, per disease) so it's a one-line
        # swap to restore once real Indonesia-sourced data exists. Each
        # component that has no sourced data is hardcoded to 0 (commented
        # inline with what it used to read from), so total_costs currently
        # reduces to just the flat direct cost. #Updated on 20260812
        final_view_creation_sql <- sprintf(
          "
          CREATE OR REPLACE TEMP VIEW %s AS
          WITH base_filtered AS (
            ...
            basic_costs AS (
              SELECT
                ...
                -- CHD basic cost components: no sourced Indonesia data for
                -- productivity/informal/complex-direct cost yet, so these
                -- are 0 (was cc.chd_prvl_prdv / cc.chd_mrtl_prdv / cc.chd_informal)
                CASE WHEN m.chd_dgns > 0 THEN 0 ELSE 0 END AS chd_prvl_prdv_costs,
                CASE WHEN m.all_cause_mrtl = 2 THEN 0 ELSE 0 END AS chd_mrtl_prdv_costs,
                CASE WHEN m.chd_dgns > 0 THEN 0 ELSE 0 END AS chd_informal_costs,

                -- Stroke basic cost components (was sc.stroke_prvl_prdv /
                -- sc.stroke_mrtl_prdv / sc.stroke_informal)
                CASE WHEN m.stroke_dgns > 0 THEN 0 ELSE 0 END AS stroke_prvl_prdv_costs,
                CASE WHEN m.all_cause_mrtl = 3 THEN 0 ELSE 0 END AS stroke_mrtl_prdv_costs,
                CASE WHEN m.stroke_dgns > 0 THEN 0 ELSE 0 END AS stroke_informal_costs,

                -- Flat per-case direct costs (USD) - the only sourced cost
                -- data currently available. All three keep the original
                -- plural _direct_costs naming (values are now the flat
                -- numbers, not the old age/sex-weighted calc for chd/stroke).
                -- Updated on 20260812
                CASE WHEN m.t2dm_dgns > 0 THEN 579 ELSE 0 END AS t2dm_direct_costs,
                CASE WHEN m.chd_dgns > 0 THEN 7338 ELSE 0 END AS chd_direct_costs,
                CASE WHEN m.stroke_dgns > 0 THEN 1685 ELSE 0 END AS stroke_direct_costs
            ...
            SELECT
              ...
              -- Aggregated direct costs (flat per-case direct costs - the
              -- only sourced direct cost data currently available)
              ...
              -- Total costs: SAME formula as before (productivity + informal
              -- + direct); productivity/informal are currently 0, so total
      ...
      create_empty_calibration_prms_file = function(replace = FALSE) {
        if (replace || !file.exists("./simulation/calibration_prms.csv")) {
          clbr <- CJ(
            ...
            chd_ftlt_clbr_fctr = 1,
            stroke_ftlt_clbr_fctr = 1,
           # t2dm_ftlt_clbr_fctr = 1, #edited 20260210 #check this with Chris
            nonmodelled_ftlt_clbr_fctr = 1
          )
      ...
      export_costs_summaries = function(duckdb_con, mcaggr, strata, ext) {
        ...
        # Define cost metrics for SELECT statement #Ask Chris: how abaout t2dm
