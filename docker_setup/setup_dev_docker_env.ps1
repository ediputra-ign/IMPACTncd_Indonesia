...
$VolumeProject   = "IMPACTncd_Indonesia_project_$SafeCurrentUser"
...
if ($UseVolumes) {
    ...
    $dockerArgs = @(
        ...
        "-v", "${VolumeProject}:/IMPACTncd_Indonesia",
    ...
    docker run -it --rm `
        ...
        -e "USER_NAME=$UserName" `
        -e "GROUP_NAME=$GroupName" `
        --mount "type=bind,source=$DockerProjectRoot,target=/IMPACTncd_Indonesia" `
