...
VOLUME_PROJECT="IMPACTncd_Indonesia_project_${CURRENT_USER}"
...
if [ "$USE_VOLUMES" = true ]; then
  ...
  docker run -it --rm \
    ...
    --mount type=volume,source="$VOLUME_PROJECT",target=/IMPACTncd_Indonesia \
  ...
  docker run -it --rm \
    ...
    -e USER_NAME="${USER_NAME}" \
    -e GROUP_NAME="${GROUP_NAME}" \
    --mount type=bind,source="$PROJECT_ROOT",target=/IMPACTncd_Indonesia \
