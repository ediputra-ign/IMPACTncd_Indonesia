...
#       Will be available as /IMPACTncd_Indonesia/scenarios inside container.
...
#     /IMPACTncd_Indonesia/scenarios inside the container, making the scenarios
...
$VolumeOutput    = "IMPACTncd_Indonesia_output_$SafeCurrentUser"
$VolumeSynthpop  = "IMPACTncd_Indonesia_synthpop_$SafeCurrentUser"
...
function Convert-PathToDockerFormat {
    param([string]$Path)
    # Input example: P:/My_Models/IMPACTncd_Indonesia
...
"@
    ...
    if ($ScenariosDir) {
        ...
        $dockerArgs += "--mount"
        $dockerArgs += "type=bind,source=$DockerScenariosDir,target=/IMPACTncd_Indonesia/scenarios"
    }
    
    # Add final arguments
    $dockerArgs += "--workdir"
    $dockerArgs += "/IMPACTncd_Indonesia"
    ...
    if ($ScenariosDir) {
        ...
        docker run -it --rm `
            ...
            --mount "type=bind,source=$DockerScenariosDir,target=/IMPACTncd_Indonesia/scenarios" `
            --workdir /IMPACTncd_Indonesia `
            $ImageName `
            bash
    } else {
        docker run -it --rm `
