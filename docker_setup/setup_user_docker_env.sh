...
#
# Scenarios Directory:
#   - If [path_to_scenarios] is provided, it will be mounted as /IMPACTncd_Indonesia/scenarios inside the container.
#
# Operation Modes:
...
VOLUME_OUTPUT_NAME="IMPACTncd_Indonesia_output_${CURRENT_USER}"
VOLUME_SYNTHPOP_NAME="IMPACTncd_Indonesia_synthpop_${CURRENT_USER}"
...
#
# Note: The Docker image already contains the /IMPACTncd_Indonesia project, so no project
...
EOF
  ...
  if [[ -n "$SCENARIOS_DIR" ]]; then
    docker run -it --rm \
      ...
      --mount type=bind,source="$SCENARIOS_DIR",target=/IMPACTncd_Indonesia/scenarios \
